...
| `/tasks` | List background tasks; `/tasks output <id>` reads a task's output, `/tasks kill <id>` cancels a running one, `/tasks clear` prunes finished tasks |
...
Type these at the start of a prompt (not as `/commands`):
