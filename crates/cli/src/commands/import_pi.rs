...
mod tests {
    ...
    #[test]
    fn parses_pi_session_header() {
        let line = r#"{"type":"session","id":"abc123","timestamp":"2026-07-01T00:00:00Z","cwd":"/home/u/proj","label":"demo"}"#;
        let entry: PiEntry = serde_json::from_str(line).unwrap();
        assert!(
            matches!(entry, PiEntry::Session(m) if m.id == "abc123" && m.label.as_deref() == Some("demo"))
        );
    }

    #[test]
    fn converts_assistant_tool_call_to_tool_use() {
        let msg: PiMessage = serde_json::from_value(serde_json::json!({
            "id": "m1",
            "message": {
                "role": "assistant",
                "model": "pi-1",
        ...
        let out = convert_message(&msg, &mut tool_calls).unwrap();
        let Message::Assistant(a) = out else {
            panic!("expected assistant message");
        };
        assert_eq!(a.stop_reason, Some(StopReason::ToolUse));
        assert_eq!(a.usage.unwrap().input_tokens, 10);
        assert_eq!(a.content.len(), 2);
        assert!(matches!(a.content[0], ContentBlock::Text { .. }));
        let (id, name, input) = a.content[1].as_tool_use().unwrap();
        assert_eq!((id, name), ("call_1", "bash"));
        assert_eq!(input["command"], "ls");
    }

    #[test]
    fn converts_tool_result_to_meta_user_message() {
        let msg: PiMessage = serde_json::from_value(serde_json::json!({
            "id": "m2",
            "message": {
                "role": "toolResult",
        ...
        assert!(matches!(
            ...
            ContentBlock::ToolResult {
                ...
                content,
                ..
            } if tool_use_id == "call_1" && content == "file list"
        ));
    }
}
