...
async fn event_loop(
    ...
    // On Termux (Android) the on-screen keyboard only appears while the
    // foreground process is blocked in a real `read()` on the pty. crossterm's
    // `EventStream` parks in `poll()` instead, so taps stop popping the soft
    // keyboard. When running under Termux, read events on a dedicated thread
    // with the blocking `event::read()` so a read is always pending.
    let mut term_events: Option<EventStream> = None;
    let mut term_rx: Option<tokio::sync::mpsc::UnboundedReceiver<Event>> = None;
    if is_termux() {
        term_rx = Some(spawn_blocking_event_reader());
    } else {
