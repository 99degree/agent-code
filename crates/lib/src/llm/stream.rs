...
/// Wait for the stream timeout, if one is configured. Provider SSE
/// loops race this against `byte_stream.next()` to bound the wait for
/// the next chunk (silent models, stalled connections). Recreating the
/// future per chunk read gives each read a fresh deadline.
