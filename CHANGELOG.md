...
- **No-output timeout for silent models / stalled streams**: the existing `api.timeout_secs` setting now bounds the wait for each chunk of the response stream, including the first byte. A model that accepts the request and then produces nothing (a saturated NIM/OpenRouter request queue, a dead connection, a silent reasoning phase) emits a clear `no output from model within Ns` error instead of hanging the agent for the full HTTP deadline. When that error arrives with zero content, the turn **retries with exponential backoff** (bounded by `api.max_retries`, default 3, and the remaining turn budget; set `api.max_retries = 0` to disable) and only fails after the budget is exhausted. Any other zero-content stream error now also fails the turn visibly instead of silently completing with an empty reply.

### Changed

- **Stream timeout default lowered to 20s** (`api.timeout_secs`): a silent model or stalled connection now fails fast instead of hanging for minutes. Reasoning models (Nemotron 3 Ultra / 3 Nano Omni, DeepSeek R1, Qwen3 with thinking, …) can legitimately stay silent for 60–120s before the first token, so raise this (e.g. 180) for reasoning-heavy usage, or set it to 0 to disable per-chunk waiting.
...
- **`/` completion now includes skills**: typing `/` and pressing Tab completes matching skill names alongside built-in commands (commands first, then skills, deduplicated) — so user/project/bundled skills invocable as `/name` are discoverable the same way commands are, not just the hard-coded command list.
- **Tab-completion for `&/skill`**: typing `&/` and pressing Tab now completes skill/workflow names (project + user + bundled), scored against what you've typed — so the background-skills feature is discoverable from the keyboard instead of requiring you to remember slugs.
- **`&/skill` runs a skill as a background task**: prefixing a skill invocation with `&` (e.g. `&/review src/`) now resolves the named skill/workflow, expands its template with the given args, and runs it as a non-blocking background task — the first production trigger for the `LocalWorkflow` kind. It shares the `&`-prefix's subprocess runner, concurrency limiter, and completion surfacing, shows up as a `LocalWorkflow` in `/tasks`, and honors `security.disable_skill_shell_execution` (fenced shell blocks are stripped before the prompt reaches the subagent). Unknown/empty slugs report a clear error instead of launching an empty agent. Reuses #319's `resolve_workflow_prompt`; new `spawn_background_workflow`.
- **Interactive background-task management from `/tasks`**: the `/tasks` slash command now manages tasks, not just lists them. `/tasks output <id>` (aliases `out`/`log`/`logs`) prints a task's captured output and `/tasks kill <id>` (aliases `stop`/`cancel`) terminates a running one — both straight from the in-process `TaskManager`, with no LLM turn burned. Previously the only way to read or cancel a background task interactively was to ask the agent to call `TaskOutput`/`TaskStop`, costing a round-trip and tokens for information already held locally.
- **`LocalWorkflow` task executor**: the `LocalWorkflow` task kind now resolves a skill slug, expands its template (substituting `{{arg}}` from the task args), and runs it as a background subagent through the same path as `LocalAgent` — instead of returning `NotImplemented`. Unknown / empty / empty-expansion slugs surface a clear error, fenced shell blocks are stripped when `security.disable_skill_shell_execution` is set, and the subagent's output is persisted so completion surfacing and `TaskOutput` show real content. New `resolve_workflow_prompt` (unit-tested without spawning) and `TaskManager::write_output`.
- **`TaskCompleted` hook + hooks now receive their context**: a new `task_completed` hook event fires when a background task (a `bash … &` job or a spawned subagent) finishes and is surfaced, so automation can chain a follow-up, post to chat, or kick CI without polling `/tasks`. More broadly, **all** hooks now actually receive their event context — previously the documented context was built but never delivered. Shell hooks get it as a JSON line on **stdin** (the convention hook-based agents use) plus `AGENT_CODE_HOOK_CONTEXT` / `AGENT_CODE_HOOK_EVENT` / `AGENT_CODE_HOOK_TOOL` env vars; HTTP hooks get it as the request body (POST). This makes agent-code's hooks genuinely scriptable/open where the closed agents aren't.
- **Subagent concurrency cap**: background subagents (the `Agent` tool's `run_in_background` path and the `&`-prefix) are now bounded by a shared `AgentExecutionLimiter` (default 4) so a lead agent can't fan out an unbounded number of subprocesses and exhaust the machine. Spawns past the cap queue and start as running ones finish (the task is registered and visible to `/tasks` while it waits). `TaskManager::spawn_command` takes an optional limiter; `AppState`/`ToolContext` carry the shared instance. Killing a task while it is still queued behind the cap discards it without ever starting its process, and subagents adopted across a restart reserve a slot (released when their process exits) so new spawns can't exceed the cap on top of survivors.
- **Turn steering — inject input into a running turn**: typing during an in-flight turn and pressing Enter now queues that text as steered input; the agent loop drains it at the next iteration boundary and injects it as a user message before the following model call (so it reads as new user input, not a split tool-use/result pair). New `QueryEngine::steer_sender()` + a per-turn steering channel, and `TurnHandle::steer(...)` so a spawned/background turn can be steered without the engine lock. The interactive REPL's in-turn key watcher buffers typed input and confirms “steering queued” on Enter (Esc/Ctrl-C still cancel).
- **Spawnable turns with observable status** (engine groundwork for promotion/steering): the query engine now publishes a coalesced `TurnStatus` (`Idle` → `Running` → `Completed`/`Aborted`/`Errored`) on a `watch` channel from a single point in `run_turn_with_sink`, without changing the turn internals. A new `query::Session` wraps the engine in a shared async mutex and runs turns via `spawn_turn`, returning a `TurnHandle` that can be polled for status, awaited, or cancelled — even while the turn owns the engine lock (cancellation goes through a lock-free shared handle). Foreground behavior is unchanged; `StreamSink` remains the rendering path.
- **Durable background tasks with adopt-on-restart**: background tasks are now journaled to `~/.cache/agent-code/tasks/<id>.json` on every transition, recording the task's status and OS pid. On interactive startup the REPL adopts any tasks left by a previous session — a task whose process is gone is reclassified `Failed` ("interrupted by restart"), one whose process is still alive stays `Running` so it can be inspected and killed, and an unfinished completion surfaces exactly once. Previously all task state was lost when the process exited. New `TaskManager::with_persistence` / `adopt`, and a `pid` field on `TaskInfo`.
